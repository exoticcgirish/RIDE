...
const {
  connectRabbitMQ,
} = require("./src/rabbitmq/connection");
...
const startServer = async () => {
  ...
  if (!mongoUri) {
    console.warn(
      "[WARN] MONGO_URI is not set. Enabling fallback mode."
    );
  ...
  if (!process.env.RABBITMQ_URL) {
    console.warn(
      "[WARN] RABBITMQ_URL is not set. RabbitMQ disabled."
    );
  } else {
    try {
      ...
      console.warn(
        "[WARN] RabbitMQ connection failed:",
        error.message
      );
    }
  }
