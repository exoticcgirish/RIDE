...
const createRideRequest = async (
  ...
  const pickupELoc =
    String(data.pickupELoc || "").trim() || null;

  const destinationELoc =
    String(data.destinationELoc || "").trim() || null;
...
const updateRideRequest =
  async (
    ...
    if (
      data.pickupELoc !== undefined
    ) {
      const pickupELoc = String(
        data.pickupELoc || "",
      ).trim();
...
const deleteRideRequest =
  async (
    ...
    return RideRequest.findOneAndDelete(
      query,
    );
  };
  ...
  async (
    ...
    return RideRequest.find(query)
      .populate({
        path: "rider",
        select:
          "name full_name phone email college",
      })
      .populate({
        path: "assignedDriver",
        select: DRIVER_FIELDS,
