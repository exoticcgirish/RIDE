...
function DriverDashboard() {
  ...
  const handleLogout = () => {
    // Clear accepted group only when this driver logs out
  ...
  // Read previously accepted group from localStorage
  ...
  const handleAcceptGroup = async (groupId) => {
    // ----------------------------------------------------------
    // IMPORTANT:
    // If driver already accepted one group, don't allow another.
    // ----------------------------------------------------------

    if (acceptedGroupId !== null) {
      console.log(
        "Driver already accepted group:",
        acceptedGroupId
      );
    ...
    try {
      ...
      // --------------------------------------------------------
      // SAVE ACCEPTED GROUP IN REACT STATE
      // --------------------------------------------------------

      setAcceptedGroupId(groupId);

      // --------------------------------------------------------
      // SAVE ACCEPTED GROUP IN LOCAL STORAGE
      // --------------------------------------------------------
      ...
      // --------------------------------------------------------
      // IMPORTANT:
      // Disable ALL accept buttons immediately.
      // --------------------------------------------------------
      ...
      setGroups((previousGroups) => {
        return previousGroups.map((group) => ({
          ...
          // Keep groups visible but mark them unavailable
          status:
            group._id === groupId
              ? "accepted"
              : group.status,
      ...
      setAcceptedGroupId(
        localStorage.getItem(ACCEPTED_GROUP_KEY) || null
      );
                  ...
                  <div className="p-5 sm:p-7">
                    ...
                    <div className="mt-7 pt-6 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto bg-[#fdbd00] hover:bg-[#efb000] disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-[#172033] px-8 py-3 rounded-xl font-extrabold transition"
                      >

                        {acceptedGroupId === group._id
                          ? "Accepted ✓"
                          : acceptedGroupId !== null
                            ? "Unavailable"
                            : acceptingId ===
