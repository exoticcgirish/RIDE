...
import { getMyRideRequests, getDriverLocation } from "../../services/rideApi";
...
function RiderDashboard({ user, onLogout }) {
  ...
  const driver =
    upcomingRide?.status === "accepted" ? upcomingRide?.assignedDriver : null;
  ...
  useEffect(() => {
    ...
    const fetchDriverLocation = async () => {
      try {
        ...
        setDriverLocationError(
          error.response?.data?.message || "Unable to get driver location.",
        );
      } finally {
        if (mounted) {
